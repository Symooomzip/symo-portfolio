...
/* Ported from reactbits.dev/animations/logo-loop, typed for this project.
   Drives the strip with requestAnimationFrame writing transform straight to
   the DOM — no React state per frame, which is what made the old
   scroll-linked marquee re-render 63 tiles on every scroll event. */
  ...
  const updateDimensions = useCallback(() => {
    ...
    if (isVertical) {
      ...
      if (h > 0) {
        ...
        setCopyCount(Math.max(MIN_COPIES, Math.ceil(viewport / h) + COPY_HEADROOM));
      }
    } else if (w > 0) {
      setSeqWidth(Math.ceil(w));
      setCopyCount(Math.max(MIN_COPIES, Math.ceil(containerWidth / w) + COPY_HEADROOM));
    }
  }, [isVertical]);
